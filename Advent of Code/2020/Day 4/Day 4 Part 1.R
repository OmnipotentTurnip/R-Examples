...
fields_lgl <- map(fields, function(x){
  grepl(x, documents)
})

names(fields_lgl) = fields

fields_lgl_df <- do.call(cbind.data.frame, fields_lgl)

fields_lgl_df$complete <- apply(fields_lgl_df, 1, prod)

print(paste0("Answer is ", sum(fields_lgl_df$complete)))
