...
answers <- map(answers, strsplit, "\n")
answers <- data.frame(gsub("\n", "", answers))
names(answers) <- "declaration"

for (i in letters){
  answers[, i] <- as.numeric(grepl(i, answers$declaration))
}

answers$yes_number <- rowSums(answers[, letters])

print(paste0("Answer is ", sum(answers$yes_number)))