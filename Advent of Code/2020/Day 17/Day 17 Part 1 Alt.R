...
z0 <- read_lines("example.txt") %>% strsplit("") %>% unlist()
...
for (j1 in colnames(z0)){
  for (j2 in rownames(z0)){
    space[[paste0("x", j1, "y", j2, "z", floor((nrow(z0)+14)/2))]] <- z0[j1, j2]
  }
...
surrounding_boxes <- function(x, y, z){
  ...
  for (i1 in -1:1){
    for (i2 in -1:1){
      for (i3 in -1:1){
          output <- c(output, space[[paste0("x", x + i1, "y", y + i2, "z", z + i3)]])
      }
    }
  }
  output
  ...
  for (k1 in (8-i):(nrow(z0) + 7 + i)){
    for (k2 in (8-i):(nrow(z0) + 7 + i)){
      for (k3 in (8-i):(nrow(z0) + 7 + i)){
        boxes <- surrounding_boxes(k1, k2, k3)
        if ((space[[paste0("x", k1, "y", k2, "z", k3)]] == "#") & (!(length(boxes[boxes == "#"]) %in% c(3, 4)))){
          new_space[[paste0("x", k1, "y", k2, "z", k3)]] <- "."
        } else {
